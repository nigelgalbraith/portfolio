...
def run_downloads_from_configs(cfg_paths: List[str]) -> bool:
    """
    ...
    for path in cfg_paths:
        ...
        result = subprocess.run(
            [sys.executable, DOWNLOAD_SCRIPT, path],
            check=True
        )
    return True
...
def downloads_status(cfg_paths: List[str]) -> bool:
    ...
    for path in cfg_paths:
        if not str(path).strip():
            continue
        r = subprocess.run(
            [sys.executable, DOWNLOAD_SCRIPT, path, "--dry-run"],
        ...
        if r.returncode != 0:
            raise RuntimeError(
                f"download_links.py dry-run failed for {path} (code {r.returncode})"
            )
    return True  
