...
STATUS_FN = check_flatpak_status
...
class FlatpakInstaller:
    ...
    def install_flatpaks(self, remote_key: str, installed_label: str) -> None:
        ...
        total = len(jobs)
        for job in jobs:
            remote = (job_block.get(job, {}) or {}).get(remote_key)
            ok = install_flatpak_app(job, remote)
            if ok:
    ...
    def uninstall_flatpaks(self, uninstalled_label: str) -> None:
        """Uninstall selected Flatpak apps; clear jobs; advance to PACKAGE_STATUS."""
        success = 0
        jobs = self.active_jobs
        total = len(jobs)
        for job in jobs:
            ok = uninstall_flatpak_app(job)
            if ok:
    ...
    def main(self) -> None:
        ...
        handlers: Dict[State, Callable[[], None]] = {
            ...
            State.INSTALL:                 lambda: self.install_flatpaks(KEY_REMOTE, INSTALLED_LABEL),
            State.UNINSTALL:               lambda: self.uninstall_flatpaks(UNINSTALLED_LABEL),
        }   
