...
Workflow:
    ...
    → CONFIG_LOADING → PACKAGE_STATUS → MENU_SELECTION → PREPARE → CONFIRM
...
# Embedded example config (used for help text on fallback and validation errors)
CONFIG_EXAMPLE = {
    "Default": {
        "ThirdParty": {
            "brave-browser": {
...
# === LOGGING ===
LOG_PREFIX      = "thirdparty"
LOG_DIR         = Path.home() / "logs" / "thirdparty"
LOGS_TO_KEEP    = 10
ROTATE_LOG_NAME = f"{LOG_PREFIX}_install_*.log"
...
class ThirdPartyInstaller:
    ...
    def detect_model(self, detection_config: Dict) -> None:
        ...
        self.tp_data = load_json(cfg_path)
    ...
    def main(self) -> None:
        handlers: Dict[State, Callable[[], None]] = {
            ...
            State.JSON_REQUIRED_KEYS_CHECK:lambda: self.validate_json_required_keys(VALIDATION_CONFIG, TP_KEY),
            ...
            State.PREPARE:                 lambda: self.prepare(SUMMARY_LABEL, ACTIONS),
