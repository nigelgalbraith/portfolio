...
class ServicesCLI:
    ...
    def select_action(self, actions: Dict[str, Dict], messages: Dict[str, str]) -> None:
        ...
        if next_state == "SHOW_LOGS":
            self.state = State.SHOW_LOGS
        else:
            self.state = State.PREPARE_PLAN

    def show_logs_state(self) -> None:
        log_paths: Dict[str, str] = {}
        for k, meta in self.model_block.items():
            p = meta.get("LogPath")
            if p:
                log_paths[k] = p
        show_logs(log_paths)
    ...
    def install_prepare_state(self) -> None:
        ...
        for key in self.jobs:
            ...
            if meta.get("ConfigSrc") and meta.get("ConfigDest"):
                copy_template(meta["ConfigSrc"], meta["ConfigDest"])
    ...
    def uninstall_remove_state(self, disabled_label: str) -> None:
        ...
        for key in self.jobs:
            ...
            if meta.get("ConfigDest"):
                remove_path(meta["ConfigDest"])
    ...
    def restart_services_state(self) -> None:
        """Restart enabled services selected in plan; → PACKAGE_STATUS."""
        success = 0
        total = len(self.jobs)
        for key in self.jobs:
            meta = self.model_block.get(key, {}) or {}
            ...
            if ok:
                ...
                log_and_print(MESSAGES["restart_fail_fmt"].format(name=name, msg=msg))
        self.finalize_msg = f"Restarted successfully: {success}/{total}"
        log_and_print(self.finalize_msg)
        self.jobs = []
        self.state = State.PACKAGE_STATUS
    ...
    def main(self) -> None:
        handlers: Dict[State, Callable[[], None]] = {
            ...
            # Utility states
            State.SHOW_LOGS:        lambda: self.show_logs_state(),
            State.RESTART_SERVICES: lambda: self.restart_services_state(),
        }
